...
def _order_dict(o: Order) -> dict:
    return {
        ...
        "eoid": o.exchange_order_id,
        "symbol": o.symbol,
        "side": o.side.value,
        "type": o.type.value,
        "q": str(o.quantity),
        "fq": str(o.filled_quantity),
...
class Repository:
    ...
    async def insert_fill(self, fill: Fill) -> None:
        await self._s.execute(
            ...
            ),
            {
                "id": fill.id,
                "oid": fill.order_id,
                "eoid": fill.exchange_order_id,
                "sym": fill.symbol,
                "side": fill.side.value,
                "q": str(fill.quantity),
                "p": str(fill.price),
                "fa": str(fill.fee.amount),
                "fc": fill.fee.currency,
                "liq": fill.liquidity,
    ...
    async def insert_market_snapshot(self, snap: MarketSnapshot) -> None:
        ...
        await self._s.execute(
            ...
            ),
            {
                ...
                "ind": json.dumps(snap.indicators or {}),
                "src": snap.source,
            },
        )
    ...
    async def list_recent_equity(self, limit=200):
        rows = (
            await self._s.execute(
                text(
                    "SELECT timestamp, total_equity FROM portfolio_snapshots ORDER BY timestamp DESC LIMIT :n"
                ),
                {"n": limit},
            )
        ).fetchall()
        ...
        *,
        ...
        await self._s.execute(
            ...
            ),
            {
                ...
                "conf": str(confidence),
                "r": rationale,
                "ro": json.dumps(raw_output),
                "m": model,
                "pv": prompt_version,
                "sv": schema_version,
                "v": 1 if validated else 0,
                "ve": json.dumps(validation_errors),
                "ts": ts,
                "trc": trace_id,
            },
        )
    ...
    async def upsert_risk_state(self, state: RiskState) -> None:
        await self._s.execute(
            ...
            {
                "k": 1 if state.killed else 0,
                "kr": state.kill_reason,
                ...
                "dp": str(state.daily_realized_pnl.amount),
                "sid": state.session_id,
                "fw": state.failures_in_window,
                "co": 1 if state.circuit_open else 0,
    ...
    async def list_recent_events(self, limit=100):
        rows = (
            await self._s.execute(text("SELECT * FROM events ORDER BY timestamp DESC LIMIT :n"), {"n": limit})
        ).fetchall()
        return [
            Event(
                id=r.id,
    ...
    async def get_user_by_username(self, username):
        row = (
            await self._s.execute(text("SELECT * FROM users WHERE username = :u"), {"u": username})
        ).first()
        return dict(row._mapping) if row else None

    async def get_user_by_id(self, user_id):
        row = (await self._s.execute(text("SELECT * FROM users WHERE id = :id"), {"id": user_id})).first()
        return dict(row._mapping) if row else None
    ...
    async def update_user_login(self, user_id, when) -> None:
        await self._s.execute(
            text("UPDATE users SET last_login_at = :w WHERE id = :id"), {"w": when, "id": user_id}
        )
    ...
    async def get_active_session(self, session_id):
        row = (
            await self._s.execute(
                text("SELECT * FROM sessions WHERE id = :id AND revoked = 0"), {"id": session_id}
            )
        ).first()
        if not row:
            return None
    ...
    async def revoke_session(self, session_id) -> None:
        await self._s.execute(text("UPDATE sessions SET revoked = 1 WHERE id = :id"), {"id": session_id})

    async def record_login_attempt(self, ip, username, success) -> None:
        await self._s.execute(
            text(
                "INSERT INTO login_attempts (ip, username, success, attempted_at) VALUES (:ip, :u, :s, :ts)"
            ),
    ...
    async def increment_failed_login(self, user_id: str) -> None:
        await self._s.execute(
            text("UPDATE users SET failed_login_count = failed_login_count + 1 WHERE id = :id"),
            {"id": user_id},
        )

    async def reset_failed_login(self, user_id: str) -> None:
        await self._s.execute(
            text("UPDATE users SET failed_login_count = 0, locked_until = NULL WHERE id = :id"),
            {"id": user_id},
        )
    ...
    async def update_user_role_status(
        self, user_id: str, *, role: str | None = None, account_status: str | None = None
    ) -> None:
    ...
    async def revoke_all_sessions_for_user(self, user_id: str) -> int:
        ...
        result = await self._s.execute(
            text("UPDATE sessions SET revoked = 1, revoked_at = :ts WHERE user_id = :uid AND revoked = 0"),

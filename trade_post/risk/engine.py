...
class RiskEngine:
    ...
    async def load(self) -> None:
        if self._repo is None:
            return
        loaded = await self._repo.get_risk_state()
    ...
    async def validate(
        ...
        cap = portfolio.total_equity * Decimal(str(self._settings.max_position_pct)) / Decimal("100")
        if requested_notional.amount > cap.amount:
    ...
    async def _persist(self) -> None:
        if self._repo is None:
            return
        try:
            await self._repo.upsert_risk_state(self._state)
